...
if args.tissue.lower() == "user_provided_files" or args.tissue.lower() == "user_provided_urls":
	assert args.user_4me1 and args.user_4me3 and args.user_27ac and args.user_27me3 and args.user_36me3 and args.user_dnase and args.user_tissue_names, "Must provide histone ChIP-seq and DNase-seq files when using the User_provided_files tissue option or URLs when using the User_provided_urls option. Must also provide the tissue/cell types names of each specified set of files."
...
for tissue in tissue_array:
	...
	if tissue == "user_provided_files" or tissue == "user_provided_urls":
		reg_element_counts_str = "Identified regions in {specified_tissue_type}:\nPutative Promoters\n{active_promoter_count} regions in an active promoter\n{bivalent_promoter_count} regions in a bivalent promoter\n{silenced_promoter_count} regions in a silenced promoter\n{unclassified_active_chromatin_gene_body_within_range_of_tss_count} regions in a gene body within an active region of chromatin, within {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n{unclassified_inactive_chromatin_gene_body_within_range_of_tss_count} regions in a gene body within an inactive region of chromatin, within {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n{unclassified_active_chromatin_within_range_of_tss_count} regions within an active region of chromatin (non-gene body), within {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n{unclassified_inactive_chromatin_within_range_of_tss_count} regions within an inactive region of chromatin (non-gene body), within {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n\nPutative Enhancers\n{active_enhancer_count} regions in an active enhancer\n{poised_enhancer_count} regions in a poised enhancer\n{primed_enhancer_count} in a primed enhancer\n{unclassified_active_chromatin_gene_body_beyond_range_of_tss_count} regions in a gene body within an active region of chromatin, beyond {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n{unclassified_inactive_chromatin_gene_body_beyond_range_of_tss_count} regions in a gene body within an inactive region of chromatin, beyond {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n{unclassified_inactive_chromatin_beyond_range_of_tss_count} regions within an inactive region of chromatin (non-gene body), beyond {tss_dist_upstream} bp upstream or {tss_dist_downstream} bp downstream of a TSS\n".format(specified_tissue_type = user_tissue_names_array[user_files_index], active_promoter_count = list(map(lambda x: x.startswith("active_promoter"), concatenated_df[last_col])).count(True), bivalent_promoter_count = list(map(lambda x: x.startswith("bivalent_promoter"), concatenated_df[last_col])).count(True), silenced_promoter_count = list(map(lambda x: x.startswith("silenced_promoter"), concatenated_df[last_col])).count(True), unclassified_active_chromatin_gene_body_within_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_active_chromatin;gene_body;within_"), concatenated_df[last_col])).count(True), unclassified_inactive_chromatin_gene_body_within_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_inactive_chromatin;gene_body;within_"), concatenated_df[last_col])).count(True), unclassified_active_chromatin_within_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_active_chromatin_within_"), concatenated_df[last_col])).count(True), unclassified_inactive_chromatin_within_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_inactive_chromatin_within_"), concatenated_df[last_col])).count(True), active_enhancer_count = list(map(lambda x: x.startswith("active_enhancer"), concatenated_df[last_col])).count(True), poised_enhancer_count = list(map(lambda x: x.startswith("poised_enhancer"), concatenated_df[last_col])).count(True), primed_enhancer_count = list(map(lambda x: x.startswith("primed_enhancer"), concatenated_df[last_col])).count(True), unclassified_active_chromatin_gene_body_beyond_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_active_chromatin;gene_body;beyond_"), concatenated_df[last_col])).count(True), unclassified_inactive_chromatin_gene_body_beyond_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_inactive_chromatin;gene_body;beyond_"), concatenated_df[last_col])).count(True), unclassified_active_chromatin_beyond_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_active_chromatin_beyond_"), concatenated_df[last_col])).count(True), unclassified_inactive_chromatin_beyond_range_of_tss_count = list(map(lambda x: x.startswith("unclassified_inactive_chromatin_beyond_"), concatenated_df[last_col])).count(True), tss_dist_upstream = str(args.tss_distance_upstream), tss_dist_downstream = str(args.tss_distance_downstream))
		with open((user_tissue_names_array[user_files_index] + "_" + args.output + "_" + "func_anno_summary.txt"), "w") as sum_file:
		...
		with open((tissue + "_" + args.output + "_" + "func_anno_summary.txt"), "w") as sum_file:
			sum_file.write(reg_element_counts_str)
		if args.verbose:
				print(reg_element_counts_str)
	...
	if tissue == "user_provided_files" or tissue == "user_provided_urls":
		func_anno_colname = user_tissue_names_array[user_files_index].upper() + "_FUNCTIONAL_ANNOTATION"
		user_files_index += 1
	else:
		func_anno_colname = tissue.upper() + "_FUNCTIONAL_ANNOTATION"
	file_input[func_anno_colname] = annotations
