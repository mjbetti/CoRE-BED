...
for (result in list.files("/home/bettimj/gamazon_rotation/mod_core-bed/uk_biobank/h2/anno_1000G", pattern = "*.results")) {
	print(result)
	gwas_name <- strsplit(result, "_h2")
	gwas_name <- unlist(lapply(gwas_name, `[[`, 1))
	gwas_names <- c(gwas_names, gwas_name)
	...
	class_vec <- c(gwas_name, df$Prop._h2)
	class_df <- rbind(class_df, class_vec)
}

names(class_df) <- c("GWAS", "Class 1", "Class 2", "Class 3", "Class 4", "Class 5", "Class 6", "Class 7", "Class 8")

class_df <- class_df[,2:9]
rownames(class_df) <- gwas_names

class_df <- as.data.frame(class_df)
class_df[,1] <- as.numeric(class_df[,1])
class_df[,2] <- as.numeric(class_df[,2])
class_df[,3] <- as.numeric(class_df[,3])
class_df[,4] <- as.numeric(class_df[,4])
class_df[,5] <- as.numeric(class_df[,5])
class_df[,6] <- as.numeric(class_df[,6])
class_df[,7] <- as.numeric(class_df[,7])
class_df[,8] <- as.numeric(class_df[,8])

png(file = "cs2g_69_traits_heatmap.png",
height=4000, width=4000, res = 300)
#margins below in inches: mai=c(xbot,xlef,xtop,xrig), mgp=axis title,axis labels,axis line
par(pty="m",mai=c(2,2,2,2),mgp=c(3,1,0))
pheatmap(class_df, display_numbers = F, color = colorRampPalette(c('white','tomato'))(100), cluster_rows = F, cluster_cols = F, show_rownames = T, show_colnames = T, fontsize_number = 3, height = 10, fontsize_col = 0.5, breaks = seq(0, 1, length.out=101), labels_col = c("Class 1", "Class 2", "Class 3", "Class 4", "Class 5", "Class 6", "Class 7", "Class 8"))
dev.off()