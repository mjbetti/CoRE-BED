...
genes_per_reg_out_dir <- "/home/bettimj/gamazon_rotation/core-bed_analysis/bayesian_priors/rcircos/gene_density/ind_tissues_genes_per_reg_element/ensid"
regs_per_gene_out_dir <- "/home/bettimj/gamazon_rotation/core-bed_analysis/bayesian_priors/rcircos/gene_density/ind_tissues_reg_elements_per_gene/ensid"
...
for (tissue in tissues) {
	...
	df <- df[!is.nan(df[,6]),]
	df <- df[(df[,6] != Inf),]
	ordered_df <- df[order(df[,6], decreasing = FALSE),]
	cat_genes <- c()
	for (i in seq(1:10)) {
		print(paste0(tissue, ":"))
		max_bin <- ordered_df[i,]
		print(max_bin)
		max_genes_df <- binned_genes_df[(binned_genes_df[,1] == max_bin[,1] & binned_genes_df[,2] == max_bin[,2] & binned_genes_df[,3] == max_bin[,3]),]
		max_genes_info <- strsplit(max_genes_df[,7], "[.]")
		gene_name <- unlist(lapply(max_genes_info, `[`, 1))
		cat_genes <- c(cat_genes, gene_name)
		print(cat_genes)
	write.table(cat_genes, file <- paste(genes_per_reg_out_dir, paste0(tissue, "_bottom_enriched_genes_per_reg_element.txt"), sep = "/"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
	}
}
...
for (tissue in tissues) {
	...
	for (i in seq(1:10)) {
		...
		cat_genes <- c(cat_genes, gene_name)
		print(cat_genes)
	write.table(cat_genes, file <- paste(regs_per_gene_out_dir, paste0(tissue, "_bottom_enriched_regs_per_gene.txt"), sep = "/"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
	}
}