...
for (tissue in tissues) {
	...
	df <- df[!is.nan(df[,6]),]
	df <- df[(df[,6] != Inf),]
	ordered_df <- df[order(df[,6], decreasing = TRUE),]
	cat_genes <- c()
	for (i in seq(1:10)) {
		print(paste0(tissue, ":"))
		...
		print(cat_genes)
	write.table(cat_genes, file <- paste(genes_per_reg_out_dir, paste0(tissue, "_top_enriched_genes_per_reg_element.txt"), sep = "/"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
	}
}
...
for (tissue in tissues) {
	...
	for (i in seq(1:10)) {
		...
		cat_genes <- c(cat_genes, gene_name)
		print(cat_genes)
	write.table(cat_genes, file <- paste(regs_per_gene_out_dir, paste0(tissue, "_top_enriched_regs_per_gene.txt"), sep = "/"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
	}
}