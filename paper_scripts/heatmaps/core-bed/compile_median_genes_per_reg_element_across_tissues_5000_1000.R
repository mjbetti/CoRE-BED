...
for (tissue in tissues) {
	...
	median_genes <- df$median_genes_per_reg_element
	new_df <- data.frame(new_df, median_genes)
}
...
write.table(new_df, file = paste(out_dir, "median_genes_per_reg_element_all_tissues_5000_1000.txt", sep = "/"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)